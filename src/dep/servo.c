  ...
  offset_from_master_filter *ofm_filt, RunTimeOpts *rtOpts, PtpClock *ptpClock,TimeInternal *correctionField)
{
  ...
  if (!rtOpts->E2E_mode)
  {
	  subTime(&ptpClock->offsetFromMaster, &ptpClock->master_to_slave_delay, &ptpClock->peerMeanPathDelay);
  }
...
void updateClock(RunTimeOpts *rtOpts, PtpClock *ptpClock)
{
  ...
  if (!rtOpts->E2E_mode)
  {
	  ...
	  ptpClock->peerMeanPathDelay.seconds, ptpClock->peerMeanPathDelay.nanoseconds);
  }
  else
  {
